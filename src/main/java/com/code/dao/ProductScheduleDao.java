package com.code.dao;

import com.code.entity.ProductSchedule;
import org.apache.ibatis.annotations.Param;
import java.util.List;

/**
 * (Productschedule)表数据库访问层
 *
 * @author makejava
 * @since 2020-05-22 08:35:08
 */
public interface ProductScheduleDao {

    /**
     * 通过ID查询单条数据
     *
     * @param psid 主键
     * @return 实例对象
     */
    ProductSchedule queryById(String psid);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<ProductSchedule> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param productschedule 实例对象
     * @return 对象列表
     */
    List<ProductSchedule> queryAll(ProductSchedule productschedule);

    /**
     * 新增数据
     *
     * @param productschedule 实例对象
     * @return 影响行数
     */
    int insert(ProductSchedule productschedule);

    /**
     * 修改数据
     *
     * @param productschedule 实例对象
     * @return 影响行数
     */
    int update(ProductSchedule productschedule);

    /**
     * 通过主键删除数据
     *
     * @param psid 主键
     * @return 影响行数
     */
    int deleteById(String psid);

}